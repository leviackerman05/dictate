...
(async()=>{
 ...
 const dir=require('path').resolve(__dirname,'../../docs/evidence/ui/windows-beta6');fs.mkdirSync(dir,{recursive:true});
 const results=[];
 ...
 await page.setViewportSize({width:152,height:22});await page.goto(`${preview}?overlay=1`);await page.waitForSelector('.recording-overlay');
 ...
 fs.writeFileSync(`${dir}/ui-check.json`,JSON.stringify({scope:'Chromium preview with synthetic IPC on macOS; not native Windows certification',results,errors,preferences:prefs,checks:['Mac-parity light and dark screens','onboarding dismissal','compact action-free recorder pebble','single modifier capture','chord capture','mouse preset','segmented preferences save','switch save','Parakeet selection','settings draft across shortcut render','save failure preserves draft and focus','dictionary draft across refresh','dictionary save','record and recovery copy']},null,2));
