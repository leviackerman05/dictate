...
(async()=>{
 ...
 window.emitModelProgress=(progress,stage)=>listeners.filter(v=>v.event==='model-progress').forEach(v=>callbacks[v.id]?.({event:v.event,payload:{progress,stage}}));
 ...
 await page.evaluate(()=>{window.mock.data.preferences.shortcut='ControlRight';window.emitState();});
 await page.waitForTimeout(100);
 await page.evaluate(()=>{window.dispatchEvent(new KeyboardEvent('keydown',{code:'ControlRight',key:'Control',ctrlKey:true}));window.dispatchEvent(new KeyboardEvent('keyup',{code:'ControlRight',key:'Control'}));});
 await page.waitForFunction(()=>window.calls.filter(call=>call.cmd==='focused_shortcut_event').length===2);
 ...
 await page.setViewportSize({width:340,height:70});await page.goto(`${preview}?overlay=1`);await page.waitForSelector('.recording-overlay');
 ...
 await page.evaluate(()=>window.emitLevel(.006));
 ...
 await page.evaluate(()=>{window.mock.ready=false;window.mock.settingUp=false;window.emitState();});
 await page.getByText('No local model',{exact:true}).waitFor();
 await page.screenshot({path:`${dir}/overlay-no-model.png`});
 await page.evaluate(()=>{window.mock.settingUp=true;window.emitState();});
 await page.getByText('Downloading local model · 0%',{exact:true}).waitFor();
 await page.evaluate(()=>window.emitModelProgress(.42,'downloading'));
 await page.getByText('Downloading local model · 42%',{exact:true}).waitFor();
 if(await page.locator('progress').getAttribute('value')!=='42')throw Error('Overlay download percentage is missing');
 await page.evaluate(()=>window.emitModelProgress(1,'loading'));
 await page.getByText('Loading local model…',{exact:true}).waitFor();
 await page.screenshot({path:`${dir}/overlay-model-loading.png`});
 await page.evaluate(()=>{window.mock.settingUp=false;window.mock.ready=true;window.emitState();});
 ...
 await page.locator('nav [data-section="models"]').click();
 await page.evaluate(()=>{window.mock.settingUp=true;window.emitState();});
 await page.locator('#model-progress').waitFor();
 await page.evaluate(()=>window.emitModelProgress(.57,'downloading'));
 await page.getByText('Downloading: 57%',{exact:true}).waitFor();
 if(await page.locator('progress[aria-label="Model setup"]').getAttribute('value')!=='57')throw Error('In-app download percentage is missing');
 await page.evaluate(()=>{window.mock.settingUp=false;window.emitState();});
